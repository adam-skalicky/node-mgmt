...
module.exports.configureAgent = async (ssh) => {
    ...
    await ssh.execCommand('rm -r ~/actions-runner/ ~/actions-runner-linux-x64-2.274.2.tar.gz ~/bin/ ~/config.sh ~/env.sh ~/run.sh ~/externals/').then((res) => (log.serverOutput(res.stdout,serverName)))
